...
The first attempt — **memdb** — was born from fascination with Raft, the consensus algorithm for distributed systems. It seemed like the most interesting part of the problem, the intellectual challenge, the piece that would make the project *real*. So I started there, before having a server, before having a client, before having a database that could store a single key-value pair. Raft is not a simple algorithm. Implementing it from scratch, alone, in spare time, was ambitious to the point of recklessness. memdb never made it past the replication layer. There was nothing underneath to replicate.

The second attempt — **raft6** — tried to approach the problem embracing the hexagonal architecture with missionary zeal. Isolate everything: the log layer, the filesystem, the event loop, the task runner, the network. Clean boundaries. Testable ports and adapters. It sounds beautiful in a conference talk. In practice, with an event loop at the core, it was an architectural nightmare. libuv wants to own its handles. The event loop *is* the application — trying to abstract it away is like trying to abstract away gravity. The project drowned in its own layers of indirection.
